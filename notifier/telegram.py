...
def notificar_vaga(job) -> bool:
    ...
    res_tg = enviar_mensagem(texto, reply_markup=_teclado_feedback(job.id))
    try:
        from notifier.dispatcher import enviar_notificacao_multicanal
        corpo_html = f"<h2>{job.titulo}</h2><p><b>Empresa:</b> {job.empresa}</p><p><b>Local:</b> {job.local}</p><p><b>Score:</b> {job.relevancia}/10</p><p><a href='{job.link}'>Ver Vaga ↗</a></p>"
        enviar_notificacao_multicanal(texto_simples=texto, assunto=f"📡 Vaga [{job.relevancia}/10]: {job.titulo}", corpo_html=corpo_html, reply_markup=_teclado_feedback(job.id))
    except Exception:
        pass
    return res_tg
