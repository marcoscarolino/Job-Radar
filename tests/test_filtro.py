...
from job import Job, RegrasFiltro, extrair_escopo_remoto
...
regras_teste_br = RegrasFiltro(
    keywords_forte=["Analista de Dados"],
    keywords_ambiguo=["Business Analyst"],
    qualificadores_dados=["sql"],
    ferramentas_titulo=["Power BI"],
    qualificadores_cargo=["analista"],
    cidades=["Recife"],
...
CASOS_COMBINA_COM = [
    ...
    ("spanish-speaking-sem-mercado-passa", "Spanish Speaking Data Analyst", "Remote", "Remoto", PERFIL_INTL.regras, True),
    ("data-analyst-latam-passa", "Data Analyst LATAM", "Remote", "Remoto", PERFIL_INTL.regras, True),
    ("sem-idioma-sem-mercado-barrada", "Senior Data Analyst", "Remote", "Remoto", PERFIL_INTL.regras, False),
    ("mercado-confirmado-dispensa-idioma-no-titulo", "Senior Data Analyst", "Remote - Espanha", "Remoto", PERFIL_INTL.regras, True),

    # --- Brasil (regras_teste_br) ---
    ("cidade-fora-da-lista-barrada", "Analista de Dados", "Nova York", "Presencial", regras_teste_br, False),
    ("cargo-fora-do-escopo-barrado", "Vendedor Externo", "Recife, PE", "Presencial", regras_teste_br, False),
...
@pytest.mark.parametrize(
    "nome,titulo,local,modalidade,regras,esperado",
    CASOS_COMBINA_COM,
    ids=[c[0] for c in CASOS_COMBINA_COM],
)
def test_combina_com(nome, titulo, local, modalidade, regras, esperado):
    job = Job(
        titulo=titulo, empresa="Teste", local=local, link=f"https://teste.invalido/{nome}",
        site="Teste", modalidade=modalidade,
    )
    assert job.combina_com(regras) == esperado
